...
const CAN_ID_ROBSTRIDE_DEFAULT: u8 = 0x7F;
...
pub(crate) const CMD_ID_READ_SINGLE_PARAMETER: u32 = 17;
const CMD_ID_WRITE_SINGLE_PARAMETER: u32 = 18;
pub(crate) const CMD_ID_FAILURE_FEEDBACK: u32 = 21;

/// radians
pub(crate) const MAX_ANGLE: f32 = 4.0 * core::f32::consts::PI;
/// radians per second
pub(crate) const MAX_VELOCITY: f32 = 15.0;
/// Nm
pub(crate) const MAX_TORQUE: f32 = 120.0;

pub const MAX_P_GAIN: f32 = 5000.0;
pub const MAX_D_GAIN: f32 = 100.0;
...
impl RobStride {
    ...
    pub fn read_parameter(&self, address: u16) -> CanBuffer {
        let extended_can_id = CMD_ID_READ_SINGLE_PARAMETER << 24 | (self.target_id as u32) << 8;
    ...
    /// CMD_ID: 18
    pub fn write_parameter(&self, address: Address) -> CanBuffer {
        let _extended_can_id = CMD_ID_WRITE_SINGLE_PARAMETER << 24 | (self.target_id as u32) << 8;

        let mut buffer = [0; 8];
        let index: u16 = address.into();
        let inner = address.as_bytes();
        ...
        CanBuffer {
            extended_can_id: _extended_can_id,
            buffer,
        }
    }
}

impl Default for RobStride {
    fn default() -> Self {
        Self {
            target_id: CAN_ID_ROBSTRIDE_DEFAULT,
        }
    }
}
